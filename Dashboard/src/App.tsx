...
const App: React.FC = () => {
  ...
  const [monitoringJira, setMonitoringJira] = useState<string | null>(null);
  ...
  const handleAnalyze = async (jiraId: string) => {
    try {
      ...
      if (apiResponse.error) {
        ...
        // Open progress monitor to show real-time updates
        setMonitoringJira(jiraId);
      }
    } catch (err) {
  ...
  return (
    <ThemeProvider>
      ...
      {monitoringJira && (
        <ProgressMonitor
          jiraNumber={monitoringJira}
          onClose={() => {
            setMonitoringJira(null);
            // Refresh data to show results
            fetchJiraData(true);
          }}
          onComplete={() => {
            // Refresh tasks list to show newly generated code from jira_prompts table
            fetchJiraData(true);
          }}
        />
      )}
